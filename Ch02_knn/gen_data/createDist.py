...
for i in range(n):
    r0,r1 = random.standard_normal(2)  # 以标准分布生成两个数据
    print(r0, r1)
    myClass = random.uniform(0,1)  # 随机生成一个数，范围为0-1
    if (myClass <= 0.16):
        fFlyer = random.uniform(22000, 60000)  # low,hight,size=none,return a scaler
        tats = 3 + 1.6*r1
        markers.append(20)
        colors.append(2.1)
...
# x,y,shape like(n,)
# s,c,shape like(n,),s-size,c-color
ax.scatter(xcord,ycord, c=colors, s=markers)  # 这个变量才有scatter函数
